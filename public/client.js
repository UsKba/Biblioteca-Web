 ...
 */
const Token = function (value, expirationTimeInSeconds, scope) {
  /* Atributos */

  var value = value;
  let startTime = new Date().getTime(); // O valor em milissegundos.
  let finishTime = new Date(startTime + expirationTimeInSeconds * 1000); // O objeto Date.
  var scope = scope;
 ...
 *
 */
const SuapClient = function (authHost, clientID, redirectURI) {
  /* Atributos privados */
  ...
  const grantType = 'implict'; // Necessário para utilizar Oauth2 com Javascript
   ...
   */
  const extractDuration = function () {
  ...
  this.getLoginURL = function () {
    const loginUrl = `${authorizationURL}?response_type=${responseType}&grant_type=${grantType}&client_id=${clientID}&redirect_uri=${redirectURI}`;
    return loginUrl;
  };
  ...
  this.getRegistrationURL = function () {
    const registrationUrl = `${authHost}/register/` + `?redirect_uri=${redirectURI}`;
